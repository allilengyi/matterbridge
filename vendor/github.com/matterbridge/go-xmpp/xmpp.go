...
func (c *Client) SendOOB(chat Chat) (n int, err error) {
	...
	stanza := "<message to='%s' type='%s' id='%s' xml:lang='en'> <body>'%s'</body>" + oobtext + thdtext + "</message>"
	
	return fmt.Fprintf(c.conn, stanza, xmlEscape(chat.Remote), xmlEscape(chat.Type),cnonce(),xmlEscape(chat.Ooburl))
}
